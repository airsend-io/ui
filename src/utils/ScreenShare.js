  ...
  getDevices() {
    return Promise.resolve()
      ...
      .then(async sources => {
        let sourceDevices = {};
        for (const source of sources) {
          sourceDevices[source.id] = {
            ...source,
            hasImage: !source.thumbnail.isEmpty(),
            image: await source.thumbnail.toDataURL()
          };
        }
...
export default class ScreenShare {
  static create(device) {
    if (isElectron() && electron) return new ElectronScreenShare();
